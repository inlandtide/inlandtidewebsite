...
function saveLeadCard(e){e.preventDefault();var fields={};groups.forEach(function(g){g[1].forEach(function(f){var el=document.getElementById('f'+f[0]);fields[f[0]]=f[2]==='checkbox'?el.checked:el.value;});});busy(true);message('Saving…');google.script.run.withSuccessHandler(function(r){show(r);message('Saved to '+r.sheet+'.');}).withFailureHandler(fail).crmSaveCard({id:current.id,revision:current.revision,fields:fields});}
form.addEventListener('submit',saveLeadCard);
